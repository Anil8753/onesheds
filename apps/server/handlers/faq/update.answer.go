...
type UpdateFAQAnswerDataPostData struct {
	WarehouseId string `json:"warehouseId" binding:"required"`
	Input       string `json:"input" binding:"required"`
	Index       int    `json:"index" binding:"required"`
}

func (s *Handler) UpdateFAQAnswer() gin.HandlerFunc {
	//
	return func(ctx *gin.Context) {
		...
		resp, err := contract.SubmitTransaction("UpdateFAQAnswer", string(outBytes))
		if err != nil {
			nethttp.ServerResponse(ctx, http.StatusInternalServerError, nethttp.ServerIssue, err)
			return
		}
